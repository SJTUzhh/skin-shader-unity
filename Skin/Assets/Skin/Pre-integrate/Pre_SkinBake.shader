Shader "Skin/Pre-Integrate_SkinBake" 
{
	Properties 
	{
	...
	CGINCLUDE
		...
		fixed4 fragBlur_5 (v2f_out In) : SV_Target
		{
			float result = 0;
			for (int i = 0;i < 5;i++)
			{
		...
		fixed4 fragBlur_3 (v2f_out In) : SV_Target
		{
			float result = 0;
			for (int i = 0;i < 3;i++)
			{
				for (int j = 0;j < 3;j++)
				{
				    half2 newUv = In.uv + float2(i - 1,j - 1) * _MainTex_TexelSize.xy * 2;
					result += tex2D(_MainTex, newUv).r * weight[i][j] ;
				}
			}
			return fixed4(result,result,result,1);
