Shader "Skin/Pre-Integrate_Skin" 
{
    Properties 
    {         
...
//			}
			...
			half4 frag_new(v2f_new i) : SV_Target
			{
				...
				// fixed4 objectNormal = tex2D(_MainNormalTex, i.uv.zw);
				// fixed3 bump = UnpackNormal(objectNormal);
				// fixed3 worldBump = normalize(mul(bump, (float3x3)unity_WorldToObject));
				
				float3 objectNormal = tex2D(_MainNormalTex, i.uv.zw).rgb - 0.5;
				objectNormal.x = -objectNormal.x;
				float3 worldBump = normalize(UnityObjectToWorldNormal(objectNormal));

				worldBump = normalize(0.45 * i.worldNormal + 0.55 * worldBump);
