Shader "Skin/Pre-Integrate_Skin_v2" 
{
    Properties 
    {         
    ...
    //                                                 dot(i.T2W2.xyz,bump)));
				...
				fixed3 objectNormal = tex2D(_MainNormalTex,i.uv.zw).rgb - 0.5;
				objectNormal.x = -objectNormal.x;
				float3 worldBump = normalize(UnityObjectToWorldNormal(objectNormal));

				// worldBump = fixed3(i.T2W0.z, i.T2W1.z,  i.T2W2.z);
				...
				//curvature
				float3 worldShapeNormal = float3(i.T2W0.z, i.T2W1.z, i.T2W2.z);
				float deltaWorldNormal = length(fwidth(worldShapeNormal));
				float deltaWorldPos = length(fwidth(worldPos));
				float curvature = (deltaWorldNormal / deltaWorldPos) * _CurveFactor * 0.01;
